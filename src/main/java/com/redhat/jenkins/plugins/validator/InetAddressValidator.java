...
public class InetAddressValidator {
	...
	public boolean isValidInet4Address(String inet4Address) {
		...
		for (String ipSegment : groups) {
			...
			if (ipSegment.length() > 1 && ipSegment.startsWith("0")) return false;
		}
		return true;
	}
	...
	public boolean isValidInet6Address(String inet6Address) {
		...
		if ((inet6Address.startsWith(":") && !inet6Address.startsWith("::")) ||
				(inet6Address.endsWith(":") && !inet6Address.endsWith("::"))) return false;
		...
		for (int index = 0; index < octets.length; index++) {
			...
			if (octet.length() == 0) {
				...
				if (octetInt < 0 || octetInt > MAX_UNSIGNED_SHORT) return false;
			}
			validOctets++;
		}
		if (validOctets > IPV6_MAX_HEX_GROUPS || (validOctets < IPV6_MAX_HEX_GROUPS && !containsCompressedZeroes)) return false;
		return true;
	}
}
